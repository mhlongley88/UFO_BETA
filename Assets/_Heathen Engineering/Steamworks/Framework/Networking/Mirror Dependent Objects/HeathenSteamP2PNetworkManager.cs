    ...
    public class HeathenSteamP2PNetworkManager : Mirror.NetworkManager
    {
        ...
        void InitializeSingleton()
        {
            ...
            if (dontDestroyOnLoad)
            {
                if (singleton != null)
                {
                    if (LogFilter.Debug) { Debug.Log("Multiple Network Managers detected in the scene. Only one NetworkManager can exist at a time. The duplicate NetworkManager will not be used."); }
                    Destroy(gameObject);
                    return;
                }
            ...
            {
                if (LogFilter.Debug) { Debug.Log("Heathen Steam P2P Network Manager created singleton (ForScene)"); }
                singleton = this;
            }
